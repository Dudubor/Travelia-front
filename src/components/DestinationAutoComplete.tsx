...
type CountryAPIItem = { pais: string; cidades: string[] };
...
export default function DestinationAutocomplete({
  ...
  // NÃO chame setState no render. Se quiser manter em estado, inicialize uma vez:
  const [countries] = useState<CountryAPIItem[]>(countriesAndCities);

  const [loading] = useState(false); // sua carga estática
  ...
  const allCities: CityRow[] = useMemo(() => {
    ...
    for (const c of countries) {
      for (const cidade of c.cidades || []) out.push({ cidade, pais: c.pais });
    }
    return out;
  }, [countries]);
              ...
              {suggestions.map((it, i) => {
                ...
                return (
                  <li
                    ...
                    className={`cursor-pointer px-3 py-2 text-sm ${i === hi ? "bg-gray-100" : ""} hover:bg-gray-50`}
