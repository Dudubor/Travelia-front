...
import { Shield, LogIn, UserPlus, LogOut, TicketsPlane } from "lucide-react";
...
export const Navbar = () => {
  ...
  const scrollToSection = (id: string) => {
    ...
    // If element not found (we're probably on another route), navigate to home then scroll
    navigate('/');
    setTimeout(() => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border/40 bg-gray-50/80 backdrop-blur-lg shadow-sm">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
      {/* Logo Section */}
      <div className="flex items-center gap-2 hover:opacity-80 transition-opacity cursor-pointer">
        <div className="p-2 rounded-lg bg-gradient-to-br from-primary to-accent shadow-md">
        <Shield className="h-5 w-5 text-primary-foreground animate-pulse" onClick={() => navigate('/')} />
        </div>
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="md:hidden flex items-center gap-1">
          {isAuthenticated ? (
          <>
            <Button size="sm" variant="ghost" onClick={() => navigate('/Viagens')} className="p-2">
